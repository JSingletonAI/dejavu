...
import { Deja VuConfigSettings } from './dejavu-types';
...
const convertToDeja VuFormat = (messages: LanguageModelV2Prompt) => {
    try {
        ...
        console.error("Error in convertToDeja VuFormat:", error);
        return [];
    }
}

const searchInternalMemories = async (query: string, config?: Deja VuConfigSettings, top_k: number = 5) => {
    try {
        ...
        const apiKey = loadApiKey({
            apiKey: (config&&config.dejavuApiKey),
        ...
        const baseUrl = config?.host || 'https://api.dejavu.ai';
...
const addMemories = async (messages: LanguageModelV2Prompt, config?: Deja VuConfigSettings) => {
    try {
        let finalMessages: Array<Message> = [];
        if (typeof messages === "string") {
            finalMessages = [{ role: "user", content: messages }];
        } else {
            finalMessages = convertToDeja VuFormat(messages);
        }
...
const updateMemories = async (messages: Array<Message>, config?: Deja VuConfigSettings) => {
    try {
        const apiKey = loadApiKey({
            apiKey: (config&&config.dejavuApiKey),
            environmentVariableName: "VENICE_API_KEY",
...
const retrieveMemories = async (prompt: LanguageModelV2Prompt | string, config?: Deja VuConfigSettings) => {
    try {
...
const getMemories = async (prompt: LanguageModelV2Prompt | string, config?: Deja VuConfigSettings) => {
    try {
        const message = typeof prompt === 'string' ? prompt : flattenPrompt(prompt);
        const memories = await searchInternalMemories(message, config);
...
const searchMemories = async (prompt: LanguageModelV2Prompt | string, config?: Deja VuConfigSettings) => {
    try {
        const message = typeof prompt === 'string' ? prompt : flattenPrompt(prompt);
        const memories = await searchInternalMemories(message, config);
        return memories;
