---
title: ElevenLabs
description: "Create voice-based conversational AI agents with ElevenLabs and Deja Vu for context-aware voice interactions."
---

Create voice-based conversational AI agents with memory capabilities by integrating ElevenLabs and Deja Vu. This integration enables persistent, context-aware voice interactions that remember past conversations.

## Overview

In this guide, we'll build a voice agent that:
1. Uses ElevenLabs Conversational AI for voice interaction
2. Leverages Deja Vu to store and retrieve memories from past conversations
...
```bash
pip install elevenlabs dejavu-memory python-dotenv
```

Configure your environment variables:

<Note>You'll need both an ElevenLabs API key and a Deja Vu API key to use this integration.</Note>
...
- `AsyncMemoryClient` from Deja Vu for memory operations
...
def main():
    ...
    if not VENICE_API_KEY:
        sys.stderr.write("VENICE_API_KEY environment variable must be set\n")
        sys.exit(1)

    # Set up Deja Vu API key in the environment
...
- Sets the Deja Vu API key in the environment for the Deja Vu client to use

### 3. Client Initialization

Initialize both the ElevenLabs and Deja Vu clients:
...
- Create an AsyncMemoryClient instance for Deja Vu interactions
...
- Stores the message in Deja Vu using the `add` method
...
When the agent identifies information worth remembering, it calls this function to store it in the Deja Vu database with the appropriate user ID.
...
4. The function stores the memory in Deja Vu linked to the user's ID
...
3. The function performs a semantic search in Deja Vu
...
By integrating ElevenLabs Conversational AI with Deja Vu, you can create voice agents that maintain context across conversations and provide personalized responses based on user history. This powerful combination enables:
