---
title: Google ADK
description: "Integrate Deja Vu with Google Agent Development Kit for persistent memory across multi-agent workflows."
---

Integrate [**Deja Vu**](https://github.com/dejavu-memory/dejavu) with [Google ADK (Agent Development Kit)](https://github.com/google/adk-python), an open-source framework for building multi-agent workflows. This integration enables agents to access persistent memory across conversations, enhancing context retention and personalization.

## Overview

1. Store and retrieve memories from Deja Vu within Google ADK agents
...
Before setting up Deja Vu with Google ADK, ensure you have:

1. Installed the required packages:
```bash
pip install google-adk dejavu-memory python-dotenv
```

2. Valid API keys:
   - <a href="https://app.dejavu.ai/dashboard/api-keys?utm_source=oss&utm_medium=integration-google-ai-adk" rel="nofollow">Deja Vu API Key</a>
   - Google AI Studio API Key

## Basic Integration Example

The following example demonstrates how to create a Google ADK agent with Deja Vu memory integration:
...
def search_memory(query: str, user_id: str) -> dict:
    ...
    memories = dejavu.search(query, filters=filters)
...
def save_memory(content: str, user_id: str) -> dict:
    ...
    try:
        result = dejavu.add([{"role": "user", "content": content}], user_id=user_id)
