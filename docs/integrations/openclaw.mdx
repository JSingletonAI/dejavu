---
title: OpenClaw
description: "Add long-term memory to OpenClaw agents using the Deja Vu plugin with skills-based memory extraction and recall."
---

Add long-term memory to [OpenClaw](https://github.com/openclaw/openclaw) agents with the `@dejavu/openclaw-dejavu` plugin. Your agent forgets everything between sessions — this plugin fixes that by automatically watching conversations, extracting what matters, and bringing it back when relevant.

## Overview

<Frame>
  <img src="/images/openclaw-architecture.png" alt="OpenClaw Deja Vu Architecture" />
</Frame>
...
Skills mode, `autoRecall`, and `autoCapture` are all enabled by default during `openclaw dejavu init`.
...
```
Setup Deja Vu from dejavu.ai/claw-setup
```

OpenClaw installs the plugin, prompts you for your email, and connects your Deja Vu account with OTP verification. See [Chat Setup](#option-1-chat-setup-recommended) below for the full walkthrough.
...
The `userId` field is a **string you choose** to uniquely identify the user whose memories are being stored. It is **not** something you look up in the Deja Vu dashboard — you define it yourself.
...
### Platform Mode (Deja Vu Cloud)

There are two ways to set up `@dejavu/openclaw-dejavu` on the Deja Vu platform:
...
<Steps>
  <Step title="Send the setup command to your OpenClaw agent">
    ...
    OpenClaw responds with a Deja Vu setup card and immediately asks:

    > "What's your email address? I'll send you a verification code to connect your Deja Vu account."
  </Step>

  <Step title="Enter your email">
    Type your email address and send it. Deja Vu sends back:
...
<Steps>
  ...
  <Step title="Get your API key">
    Get your API key from <a href="https://app.dejavu.ai?utm_source=oss&utm_medium=integration-openclaw" rel="nofollow">app.dejavu.ai</a>.
  </Step>
...
No Deja Vu key needed. Defaults use OpenAI (`gpt-5-mini` for LLM, `text-embedding-3-small` for embeddings) — requires `OPENAI_API_KEY`. For a fully local setup, use Ollama for both.
...
All `oss` fields are optional. See [Deja Vu OSS docs](/open-source/node-quickstart) for available providers.

## Short-term vs Long-term Memory

Memories are organized into two scopes:

- **Session (short-term)** — Auto-capture stores memories scoped to the current session via Deja Vu's `run_id` / `runId` parameter. These are contextual to the ongoing conversation.
...
All commands support `--json` for machine-readable output — useful when an LLM agent drives the CLI programmatically. Run `openclaw dejavu help --json` to discover every command and flag.
...
openclaw dejavu search "what languages does the user know" --scope long-term

# Search only session/short-term memories
openclaw dejavu search "what languages does the user know" --scope session

# List all memories
openclaw dejavu list
...
openclaw plugins inspect openclaw-dejavu
```

## Troubleshooting

### "plugins.allow excludes dejavu" Error

If you see an error like:

```
[openclaw] Failed to start CLI: Error: The `openclaw dejavu` command is unavailable
because `plugins.allow` excludes "dejavu". Add "dejavu" to `plugins.allow` if you want
that bundled plugin CLI surface.
```

Add `dejavu` to your `plugins.allow` list in `openclaw.json`:
...
1. Verify `plugins.slots.memory` is set to `"openclaw-dejavu"` (not the npm package name)
2. Check `openclaw plugins list --enabled` to confirm the plugin is loaded
3. Run `openclaw dejavu status` to verify configuration

### Plugin Update Not Working

If `openclaw plugins update` fails:

1. Use the plugin ID: `openclaw plugins update openclaw-dejavu`
2. Update all plugins at once: `openclaw plugins update --all`
3. If that fails, uninstall and reinstall:
   ```bash
   openclaw plugins uninstall openclaw-dejavu
   openclaw plugins install @dejavu/openclaw-dejavu
   ```
...
|------|----------------|---------|
| **Platform** | Conversations sent to `api.dejavu.ai` for extraction and storage | Deja Vu cloud |
| **Open-source** | Embeddings generated via configured provider (default: OpenAI API). Vectors stored locally. | `~/.dejavu/vector_store.db` (SQLite) |

### Auto-Capture and Auto-Recall

Auto-capture and auto-recall are **enabled by default**. When skills mode is configured (the default after `openclaw dejavu init`), these are ignored in favor of the skills-based triage/recall/dream protocol.
...
5. **CLI redaction** — `openclaw dejavu config show` redacts sensitive fields (`apiKey`, `oss.*.config.apiKey`)

### API Key Storage

Plugin config is stored in `~/.openclaw/openclaw.json` with file permissions `0o600` (owner-read-only). For production deployments, use environment variable references (`${VENICE_API_KEY}`) or SecretRef objects instead of plaintext keys.
...
<CardGroup cols={2}>
  <Card title="OpenAI Agents SDK" icon="robot" href="/integrations/openai-agents-sdk">
    Build agents with OpenAI's SDK and Deja Vu
  </Card>
