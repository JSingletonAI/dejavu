---
title: ChatDev
description: "Add persistent, cloud-managed memory to ChatDev multi-agent workflows with Deja Vu — no code required, just YAML configuration."
---

Build multi-agent workflows in [ChatDev](https://github.com/OpenBMB/ChatDev) with persistent memory powered by Deja Vu. ChatDev is a zero-code multi-agent platform where agents, tools, and workflows are defined entirely in YAML. Deja Vu integrates as a built-in memory store (`type: dejavu`), giving your agents cloud-managed semantic search and cross-session persistence — all without writing any code.

## Overview

In this guide, you'll:
1. Set up ChatDev with the Deja Vu memory store
...
- A **Deja Vu API key** from <a href="https://app.dejavu.ai?utm_source=oss&utm_medium=integration-chatdev" rel="nofollow">app.dejavu.ai</a>
- An **OpenAI API key** (or another LLM provider supported by ChatDev)

## Setup and Configuration

Install ChatDev and its dependencies (includes `dejavu-memory`):
...
<Note>Get your Deja Vu API key from <a href="https://app.dejavu.ai?utm_source=oss&utm_medium=integration-chatdev" rel="nofollow">Deja Vu Platform</a>.</Note>
...
## Configure Deja Vu Memory Store

In your ChatDev workflow YAML, add a Deja Vu memory store in the `memory` section:
...
Deja Vu handles all storage, embeddings, and search server-side — no local vector databases or embedding models are needed.
...
version: 0.4.0
graph:
  description: Memory-backed conversation using Deja Vu
...
uv run python run.py --path yaml_instance/demo_dejavu_memory.yaml --name my_project
...
When an agent with Deja Vu memory receives input, the following cycle runs automatically:

**1. Retrieve** — Before generating a response, ChatDev queries Deja Vu with the user's input using semantic search. Relevant memories are injected into the agent's context in this format:
...
**3. Store** — After generation, the user's input is sent to Deja Vu via `client.add()`. Deja Vu's extraction model automatically identifies and stores facts, preferences, and key information. Only user input is stored — agent output is excluded to keep memories clean.

Memories persist in Deja Vu's cloud across all sessions. The next time the same `user_id` or `agent_id` is used, previous memories are automatically retrieved.

## Dual-Scope Memory (User + Agent)

When both `user_id` and `agent_id` are configured, Deja Vu uses an OR filter to search across both scopes in a single query:
...
|-------|----------|-------------|
| `api_key` | Yes | Deja Vu API key from <a href="https://app.dejavu.ai?utm_source=oss&utm_medium=integration-chatdev" rel="nofollow">app.dejavu.ai</a> |
...
- **`dejavu-memory` not installed** — If you see `ImportError: dejavu-memory is required for Deja VuMemory`, run `uv add dejavu-memory` or `pip install dejavu-memory` to add the dependency.
- **Invalid API key** — A wrong or expired `VENICE_API_KEY` will log errors like `Deja Vu search failed` or `Deja Vu add failed` but won't crash the agent. Check your key at <a href="https://app.dejavu.ai?utm_source=oss&utm_medium=integration-chatdev" rel="nofollow">app.dejavu.ai</a>.
- **Pipeline headers in memories** — ChatDev automatically strips internal pipeline headers (e.g., `=== INPUT FROM TASK (user) ===`) before sending text to Deja Vu, so your memories stay clean.
- **Clearing test memories** — To delete memories created during testing, use the Deja Vu dashboard at <a href="https://app.dejavu.ai?utm_source=oss&utm_medium=integration-chatdev" rel="nofollow">app.dejavu.ai</a> or the Python SDK: `MemoryClient().delete_all(user_id="your-test-user")`.

## Key Features

1. **Zero-Code Integration** — Configure Deja Vu entirely through YAML, no Python code required
2. **Cloud-Managed Storage** — Deja Vu handles embeddings, persistence, and search server-side
...
By adding Deja Vu as a memory store in ChatDev, your multi-agent workflows gain persistent, intelligent memory with zero code changes. Agents automatically remember past interactions and use that context to provide personalized, coherent responses across sessions.

<CardGroup cols={2}>
  <Card title="CrewAI Integration" icon="users" href="/integrations/crewai">
    Build multi-agent systems with CrewAI and Deja Vu
  </Card>
  <Card title="AutoGen Integration" icon="robot" href="/integrations/autogen">
    Build conversational agents with AutoGen and Deja Vu
  </Card>
</CardGroup>
