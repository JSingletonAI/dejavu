---
title: Cursor
description: "Add persistent memory to Cursor with the Deja Vu plugin — MCP server, lifecycle hooks, and SDK skill for context-aware coding."
---

Add persistent memory to [**Cursor**](https://cursor.com) with the Deja Vu plugin. Your AI assistant forgets everything between sessions — this plugin fixes that by connecting to Deja Vu's cloud memory layer via MCP, automatically capturing learnings at key lifecycle points, and retrieving relevant context before every response.
...
3. **SDK Skill** — Teaches the agent how to integrate the Deja Vu SDK into your applications
4. **Zero local dependencies** — Cloud-hosted MCP server, no local setup required

## Prerequisites

Before setting up Deja Vu with Cursor, ensure you have:

1. A Deja Vu Platform account and API key:
   - <a href="https://app.dejavu.ai?utm_source=oss&utm_medium=integration-cursor" rel="nofollow">Sign up at app.dejavu.ai</a>
   - <a href="https://app.dejavu.ai/dashboard/api-keys?utm_source=oss&utm_medium=integration-cursor" rel="nofollow">Get your API key</a> (starts with `m0-`)
...
<Warning>
  Already have `dejavu` configured as an MCP server in Cursor? Remove the existing entry from your Cursor MCP settings before installing to avoid duplicate tools.
</Warning>
...
The fastest way to get started. Click the link below to install the Deja Vu MCP server directly in Cursor:
...
Install from the [Cursor Marketplace](https://cursor.com/marketplace) for the complete experience including lifecycle hooks, the Deja Vu SDK skill, and automatic memory capture.

<Info icon="check">
  Start a new Cursor session and ask: *"List my dejavu entities"* or *"Search my memories for hello"*. If the `dejavu` tools appear and respond, you're all set.
</Info>
...
When installed via the Cursor Marketplace, Deja Vu hooks into Cursor's lifecycle:

### Session Start
On every new session, the plugin prompts the agent to call `search_memories` to load relevant context from prior sessions.

### User Prompt
Before processing each user message, the plugin searches Deja Vu for relevant memories and injects them into context. Short prompts are skipped to minimize latency.
...
- **"Connection failed"** — Verify `VENICE_API_KEY` is set: `echo $VENICE_API_KEY`
- **Duplicate tools** — If you had a previous MCP config for `dejavu`, remove it before installing the plugin
- **No tools appearing** — Go to Cursor Settings > MCP and verify the `dejavu` server shows as connected
