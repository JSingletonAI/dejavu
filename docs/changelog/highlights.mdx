---
title: "Highlights"
description: "Major product launches, headline features, and milestones for Deja Vu."
mode: "wide"
---

<Update label="2026-05-13" description="Temporal Reasoning for Deja Vu Platform v3">

**Temporal Reasoning — Time-Aware Retrieval for Platform v3**

Deja Vu Platform v3 can now interpret time-aware memories and queries so assistants retrieve the right information for questions about the past, upcoming plans, and current state.
...
<Update label="2026-04-14" description="Deja Vu SDK v2.0.0 / v3.0.0">
...
<Update label="2026-04-06" description="Deja Vu Skill Graph">

**Deja Vu Skill Graph — In-Context Documentation for AI Agents**

AI coding agents in Claude Code, Cursor, and Codex can now access Deja Vu knowledge directly in their workflow — no doc searching required. Three interconnected skills launched:

- **dejavu Core Skill** — Complete Python and TypeScript SDK reference, REST API patterns, and integration guides for LangChain, CrewAI, Autogen, and more
- **dejavu-cli Skill** — Terminal command reference, configuration walkthroughs, and CI/CD recipes
- **dejavu-vercel-ai-sdk Skill** — Vercel AI SDK provider API, memory-augmented generation patterns, and multi-provider setup

</Update>

<Update label="2026-04-06" description="Deja Vu CLI v0.2.2">

**Official Deja Vu CLI — Now on PyPI and npm**

A full-featured command-line interface for Deja Vu, available in both Python and Node.js:

- **Install:** `pip install dejavu-cli` or `npm install -g @dejavu/cli`
- **Full command suite** — `add`, `search`, `list`, `get`, `update`, `delete`, `import`, `config`, `init`, `status`, `entity`, `event`
- **Interactive setup** — `dejavu init` with email verification or direct API key entry
...
The OpenClaw Deja Vu plugin went from initial release to production-ready in one week (v1.0.0 → v1.0.4):
...
- **Interactive CLI** — `openclaw dejavu init`, `status`, `config`, `import`, and `event` commands
...
<Update label="2026-04-02" description="Deja Vu Plugin for AI Editors">

**Deja Vu Plugin for Claude Code, Cursor, and Codex**

Launched a unified Deja Vu plugin across three major AI development environments — Claude Code and Cursor first (March 25), then Codex (April 2):

- **9 MCP memory tools** — add, search, get, update, delete, bulk delete, entity management via `mcp.dejavu.ai`
...
<Update label="2026-03-14" description="Deja Vu Platform Skill">

**Deja Vu Platform Skill on skills.sh**

First skill launch — a dedicated Deja Vu skill providing platform API reference, quickstart patterns, and integration examples directly inside agent sessions. Available on [skills.sh](https://skills.sh) for any compatible AI coding agent.
