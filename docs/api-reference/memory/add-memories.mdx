...
| --- | --- | --- |
| `Authorization: Token <VENICE_API_KEY>` | Yes | API key scoped to your workspace. |
| `Accept: application/json` | Yes | Ensures a JSON response. |

## Request body

Provide conversation messages for Deja Vu to extract memories from. At least one entity ID (`user_id`, `agent_id`, `app_id`, or `run_id`) is required so the memory is scoped to a session. Entity IDs are accepted at the top level.
...
| --- | --- | --- | --- |
| `messages` | array | Yes | Conversation turns for Deja Vu to extract memories from. Each object should include `role` and `content`. |
...
```json 400 response
{
  ...
  "details": {
    "message": "Invalid input data. Please refer to the memory creation documentation at https://docs.dejavu.ai/platform/quickstart#4-1-create-memories for correct formatting and required fields."
  }
}
```
</CodeGroup>
