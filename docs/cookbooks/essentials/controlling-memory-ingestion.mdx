...
Deja Vu lets you control your memory ingestion pipeline. In this cookbook, we'll demonstrate these controls using a medical assistant example - showing how to filter unwanted data, enforce data formats, and implement confidence-based storage.
...
Deja Vu provides **three tools to control** what gets stored:
...
<Note>
Replace `your-api-key` with your actual Deja Vu API key from the <a href="https://app.dejavu.ai?utm_source=oss&utm_medium=cookbook-memory-ingestion" rel="nofollow">dashboard</a>. Without proper API authentication, memory operations will fail.
</Note>
...
Custom instructions tell Deja Vu what to store and what to ignore.
...
Deja Vu assigns confidence scores to extracted memories. Use these to filter low-quality data.
...
| --- | --- | --- | --- |
| `infer=True` *(default)* | Runs the LLM pipeline so Deja Vu extracts structured facts and resolves conflicts automatically. | Daily conversations, preference tracking, anything you want deduped. | Slightly slower because inference runs on every write. |
...
# Initialize client
client = MemoryClient(api_key=os.getenv("VENICE_API_KEY"))
