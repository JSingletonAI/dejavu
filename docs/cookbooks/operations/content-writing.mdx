...
This guide demonstrates how to leverage **Deja Vu** to streamline content writing by applying your unique writing style and preferences using persistent memory.

## Why Use Deja Vu?

Integrating Deja Vu into your writing workflow helps you:
...
# Set up Deja Vu and OpenAI client
client = MemoryClient()
openai = OpenAI()
...
Here we use **Deja Vu open source** (`Memory`): all local, no API keys needed for memory. Vectors in **Qdrant**, LLM and embeddings via **Ollama**. The **OpenAI** Python SDK calls Ollama's **OpenAI-compatible** `/v1` endpoint for content rewriting.
...
## Storing Your Writing Preferences in Deja Vu

<Tabs>
  <Tab title="Platform">
```python
def store_writing_preferences():
    """Store your writing preferences in Deja Vu."""
...
def apply_writing_style(original_content):
    """Use preferences stored in Deja Vu to guide content rewriting."""
...
def content_writing_workflow(content):
    ...
    # Edit the document with Deja Vu preferences
...
Deja Vu enables a seamless, intelligent content-writing workflow, perfect for content creators, marketers, and technical writers looking to scale their personal tone and structure across work.
...
<CardGroup cols={2}>
  ...
  <Card title="Email Automation with Deja Vu" icon="envelope" href="/cookbooks/operations/email-automation">
