---
title: Search Memory
description: Retrieve relevant memories from Deja Vu using powerful semantic and filtered search capabilities.
icon: "magnifying-glass"
iconType: "solid"
---

# How Deja Vu Searches Memory

Deja Vu's search operation lets agents ask natural-language questions and get back the memories that matter most. Like a smart librarian, it finds exactly what you need from everything you've stored.
...
Deja Vu cleans and enriches your natural-language query so the downstream embedding search is accurate.
</Step>
...
## Search with Deja Vu Open Source
...
<Note>
  On Deja Vu Platform v3, time-aware queries use Temporal Reasoning internally while preserving the normal search response shape. See <Link href="/platform/features/temporal-reasoning">Temporal Reasoning</Link>.
</Note>
...
- **Use natural language**: Deja Vu understands intent, so describe what you're looking for naturally
...
<Callout type="tip" icon="plug">
  **MCP Alternative**: With <Link href="/platform/dejavu-mcp">Deja Vu MCP</Link>, AI agents can search their own memories proactively when needed.
</Callout>
...
- <Link href="/cookbooks/operations/support-inbox">Support Inbox with Deja Vu</Link> demonstrates scoped search with rerankers.
- <Link href="/cookbooks/integrations/tavily-search">Tavily Search with Deja Vu</Link> shows hybrid search in action.
