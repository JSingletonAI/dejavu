...
Deja Vu’s update operation lets you fix or enrich an existing memory without deleting it. When a user changes their preference or clarifies a fact, use update to keep the knowledge base fresh.
...
Call `update` (or `batch_update`) with new text and optional metadata. Deja Vu overwrites the stored value and adjusts indexes.
</Step>
<Step title="Verify">
...
<Callout type="tip" icon="plug">
  **MCP Alternative**: With <Link href="/platform/dejavu-mcp">Deja Vu MCP</Link>, AI agents can update their own memories when users correct information.
</Callout>
...
- <Link href="/cookbooks/operations/support-inbox">Support Inbox with Deja Vu</Link> uses updates to refine customer profiles.
- <Link href="/cookbooks/companions/ai-tutor">AI Tutor with Deja Vu</Link> demonstrates user preference corrections mid-course.
