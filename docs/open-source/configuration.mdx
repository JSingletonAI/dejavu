...
description: "Wire up Deja Vu OSS with your preferred LLM, vector store, embedder, and reranker."
icon: "sliders"
---

# Configure Deja Vu OSS Components
...
<Tabs>
  <Tab title="Python">
<Steps>
<Step title="Install Deja Vu OSS">
```bash
...
cd dejavu/examples/docker-compose
```
</Step>
...
- `Unknown reranker` → update the SDK (`pip install --upgrade dejavu-memory`) to load the latest provider registry.
