...
description: Tailor fact extraction so Deja Vu stores only the details you care about.
icon: "wand-magic-sparkles"
---

Custom instructions let you decide exactly which facts Deja Vu records from a conversation. Define a focused prompt, give a few examples, and Deja Vu will add only the memories that match your use case.
...
- **Structured output:** Responses return JSON with a `facts` array that Deja Vu converts into individual memories.
...
<CardGroup cols={2}>
  <Card title="Review Add Operations" icon="list" href="/core-concepts/memory-operations/add">
    Refresh how Deja Vu stores memories and how prompts influence fact creation.
  </Card>
