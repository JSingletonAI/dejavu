---
title: "Overview"
description: "Self-host Deja Vu with full control over your infrastructure and data"
icon: "house"
---

# Deja Vu Open Source Overview

Deja Vu Open Source delivers the same adaptive memory engine as the platform, but packaged for teams that need to run everything on their own infrastructure. You own the stack, the data, and the customizations.

## What Deja Vu OSS provides
...
<Info>
  Two ways to run Deja Vu OSS: as a **library** inside your app (Python or Node), or as a **self-hosted server** with a dashboard, per-user API keys, and a request audit log.
</Info>
...
<CardGroup cols={3}>
  ...
  <Card title="Memory Evaluation" icon="chart-line" href="/core-concepts/memory-evaluation">
    Benchmarks and how Deja Vu is tested.
  </Card>
</CardGroup>
...
<AccordionGroup>
  <Accordion title="What you get with Deja Vu OSS" icon="code-branch">
...
<Note>
  **Library defaults** (when you `import` Deja Vu and call `Memory()` directly):
  - LLM: OpenAI `gpt-5-mini` (via `OPENAI_API_KEY`)
  - Embeddings: OpenAI `text-embedding-3-small`
  - Vector store: Local Qdrant at `/tmp/qdrant`
  - History store: SQLite at `~/.dejavu/history.db`
...
<Note>
  **Self-hosted server defaults** (the `server/` Docker Compose stack):
  - LLM: OpenAI `gpt-4.1-nano-2025-04-14` (override with `DEJAVU_DEFAULT_LLM_MODEL`)
  - Embeddings: OpenAI `text-embedding-3-small` (override with `DEJAVU_DEFAULT_EMBEDDER_MODEL`)
