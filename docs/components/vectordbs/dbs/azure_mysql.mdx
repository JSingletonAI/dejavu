---
title: Azure MySQL
description: "Use Azure Database for MySQL as a vector store in Deja Vu with JSON-based vector storage for semantic search."
---
...
| `collection_name` | Table name for storing vectors | `"dejavu"` |
...
az group create --name dejavu-rg --location eastus

# Create MySQL Flexible Server
az mysql flexible-server create \
    --resource-group dejavu-rg \
    --name dejavu-mysql-server \
    --location eastus \
    --admin-user myadmin \
...
az mysql flexible-server db create \
    --resource-group dejavu-rg \
    --server-name dejavu-mysql-server \
    --database-name dejavu_db
...
GRANT ALL PRIVILEGES ON dejavu_db.* TO 'your-app-identity'@'%';
FLUSH PRIVILEGES;
```
