...
description: "Migrate your Deja Vu Open Source implementation to Deja Vu Platform for managed infrastructure and advanced features."
icon: "cloud-arrow-up"
...
Move your Deja Vu implementation to managed infrastructure with enterprise features.
...
<Info>
  Using Deja Vu Open Source with **hosted Qdrant**? You can migrate your existing memories to Deja Vu Platform with a one-line script below.
</Info>
...
1. **Sign up**: Create an account on <a href="https://app.dejavu.ai?utm_source=oss&utm_medium=migration-oss-to-platform" rel="nofollow">Deja Vu Platform</a>.
...
If your Deja Vu Open Source setup uses **hosted Qdrant** as the vector store, you can import your existing memories to Deja Vu Platform with one command:

```bash
curl -fsSL https://raw.githubusercontent.com/dejavu-memory/dejavu/main/scripts/oss-to-platform-migrate.sh | bash
```
...
If you are using a different vector store and want to migrate to Platform, please contact Deja Vu support and we’ll send you a custom migration script for your setup.
...
```bash
pip install dejavu-memory --upgrade
```
...
# Set VENICE_API_KEY in environment or pass explicitly
...
<AccordionGroup>
  ...
  <Accordion title="Webhooks for Real-time Events">
    ...
    webhook = client.webhooks.create(
        project_id="proj_123",
        name="Memory Events",
        url="https://your-app.com/webhooks/dejavu",
...
- <a href="https://app.dejavu.ai?utm_source=oss&utm_medium=migration-oss-to-platform" rel="nofollow">Platform Dashboard</a> - Monitor usage and manage settings.
