...
description: "Guide for Deja Vu Platform users to adopt the new memory algorithm with single-pass extraction, entity linking, and multi-signal retrieval."
icon: "arrow-right"
iconType: "solid"
---

<Info>
**No action required for most users.** The new algorithm is rolling out automatically to all Deja Vu Platform projects. This guide covers what changed, what to expect, and how to take full advantage of the new capabilities.
</Info>

## Overview

The new Deja Vu memory algorithm is a ground-up redesign of how memories are extracted, stored, and retrieved. It scores **91.6 on LoCoMo** and **93.4 on LongMemEval** — a +20 and +26 point improvement over the previous algorithm — while cutting extraction latency roughly in half.
...
```bash cURL
# Add memories
curl -X POST https://api.dejavu.ai/v3/memories/add/ \
...
# List memories (paginated)
curl -X POST 'https://api.dejavu.ai/v3/memories/?page=1&page_size=50' \
  -H "Authorization: Token your-api-key" \
  -H "Content-Type: application/json" \
...
- Email us at [support@dejavu.ai](mailto:support@dejavu.ai)
