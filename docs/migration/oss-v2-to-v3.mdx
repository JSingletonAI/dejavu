...
description: "Guide for self-hosted Deja Vu users to upgrade to the new memory algorithm with ADD-only extraction, hybrid search, and entity linking."
icon: "arrow-right"
iconType: "solid"
---
...
The new Deja Vu release redesigns both extraction and retrieval, and cleans up the SDK surface across Python and TypeScript:
...
| `messages` in `add()` | Could be `None` | Must be `str` / `dict` / `list[dict]` — other types raise `Deja VuValidationError` (code `VALIDATION_003`) | Always pass a string, dict, or list of messages |
...
<Tabs>
  <Tab title="Python">
    ```bash
    # Basic upgrade
    pip install --upgrade dejavu-memory

    # For hybrid search + entity extraction (recommended)
    pip install --upgrade "dejavu-memory[nlp]"
    ...
    <Info>
    **Supported Python versions for `[nlp]` extras: 3.10 – 3.12.** spaCy and its `blis` / `thinc` dependencies do not yet ship prebuilt wheels for Python 3.13, so installs on 3.13 will fail at build time. Use Python 3.12 (or older) for the `[nlp]` extras until upstream support lands. The base `dejavu-memory` package works on all supported Python versions; only the NLP extras are constrained.
    </Info>
  </Tab>
  <Tab title="TypeScript">
    ```bash
    npm install dejavu-memory@latest
    ```
  </Tab>
</Tabs>

<Info>
The Python `[nlp]` extra installs [spaCy](https://spacy.io/) for entity extraction and keyword lemmatization. Without it, Deja Vu still works but falls back to semantic-only search (no entity linking, no BM25 lemmatization).
</Info>
...
<Tabs>
  ...
  <Tab title="Python Client SDK">
    ...
    from dejavu.client.types import SearchMemoryOptions
...
<Tabs>
  ...
  <Tab title="Python Client SDK">
    ```python
    from dejavu.client.types import AddMemoryOptions
...
- Uninstall graph drivers (neo4j, memgraph, etc.) if you were using them only for Deja Vu
...
|---|---|---|
| spaCy (`dejavu-memory[nlp]`) | No entity extraction, no BM25 lemmatization | Yes (semantic-only) |
...
- Join our [Discord community](https://dejavu.ai/discord) for real-time support
- Open an issue on [GitHub](https://github.com/dejavu-memory/dejavu/issues)
