...
class EmbedChain(JSONSerializable):
    ...
    def chat(
        ...
        **kwargs: dict[str, Any],
        ...
        memories = None
        if self.dejavu_memory:
            memories = self.dejavu_memory.search(
        ...
        # Add to Deja Vu memory if enabled
        # Adding answer here because it would be much useful than input question itself
        if self.dejavu_memory:
            self.dejavu_memory.add(data=answer, agent_id=self.config.id, user_id=session_id)
