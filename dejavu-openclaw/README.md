# @dejavu/openclaw-dejavu

Long-term memory for [OpenClaw](https://github.com/openclaw/openclaw) agents, powered by [Deja Vu](https://dejavu.ai).
...
By default, the plugin runs in **skills mode**: the agent controls what to remember (triage), how to recall (recall), and periodic cleanup (dream). Skills mode, `autoRecall`, and `autoCapture` are all enabled by default during `openclaw dejavu init`.
...
If you're an AI agent setting up Deja Vu autonomously (no human dashboard signup), mint an evaluation API key in <5 seconds:

```bash
dejavu init --agent --json
```

No email, no OTP, no browser. The key is written to your Deja Vu CLI config and exported as `VENICE_API_KEY` — pass it to `openclaw dejavu init --api-key $VENICE_API_KEY --user-id <id>` to wire OpenClaw up immediately. The human owner can later run `dejavu init --email <their-email>` to claim ownership: memories transfer, same API key keeps working, no agent disruption.

Humans should follow the Quick Start below.

## Quick Start

### Platform (Deja Vu Cloud)
...
1. **Install the plugin via the OpenClaw CLI:**
   ...
   ```bash
   openclaw plugins install @dejavu/openclaw-dejavu
   ```

2. **Get your API key** from [app.dejavu.ai](https://app.dejavu.ai/dashboard/api-keys?utm_source=oss&utm_medium=openclaw-readme).

3. **Select the plugin as your memory backend in `openclaw.json`.** Either initialize via the CLI:

   ```bash
   openclaw dejavu init --api-key <your-key> --user-id <your-user-id>
   ```
...
```bash
openclaw plugins update openclaw-dejavu
```

### Open-Source (Self-hosted)

No Deja Vu key needed. Vectors are stored locally in SQLite at `~/.dejavu/vector_store.db` — no external database required.
...
```bash
openclaw dejavu init --mode open-source
```
...
openclaw dejavu init --mode open-source --oss-llm ollama --oss-embedder ollama --oss-vector qdrant --json
```
...
All `oss` fields are optional. See the [Deja Vu OSS docs](https://docs.dejavu.ai/open-source/node-quickstart) for supported providers.

## How It Works

<p align="center">
  <img src="https://raw.githubusercontent.com/dejavu-memory/dejavu/main/docs/images/openclaw-architecture.png" alt="Architecture" width="800" />
</p>

### Skills Mode (Default)

Enabled automatically during `openclaw dejavu init`. The agent controls memory through three skills:
...
- **Auto-Recall** — Before the agent responds, the plugin searches Deja Vu for relevant memories and injects them into context.
- **Auto-Capture** — After the agent responds, the conversation is filtered through a noise-removal pipeline and sent to Deja Vu. New facts get stored, stale ones updated, duplicates merged.
...
All commands: `openclaw dejavu <command>`. All commands support `--json` for machine-readable output (for LLM agents).

```bash
# Memory operations
openclaw dejavu add "User prefers TypeScript over JavaScript"
openclaw dejavu search "what languages does the user know"
openclaw dejavu search "preferences" --scope long-term
openclaw dejavu get <memory_id>
openclaw dejavu list --user-id alice --top-k 20
openclaw dejavu update <memory_id> "Updated preference text"
openclaw dejavu delete <memory_id>
openclaw dejavu delete --all --user-id alice --confirm
openclaw dejavu import memories.json

# Management
openclaw dejavu init                                          # interactive setup
openclaw dejavu init --mode open-source --oss-llm ollama      # non-interactive OSS
openclaw dejavu init --api-key <key> --user-id alice          # non-interactive platform
openclaw dejavu status
openclaw dejavu config show
openclaw dejavu config get api_key
openclaw dejavu config set user_id alice

# Events (platform only)
openclaw dejavu event list
openclaw dejavu event status <event_id>

# Memory consolidation
openclaw dejavu dream
openclaw dejavu dream --dry-run

# JSON output (any command)
openclaw dejavu search "preferences" --json
openclaw dejavu list --json
openclaw dejavu status --json
openclaw dejavu help --json                                   # discover all commands + flags
```
...
Enabled by default during `openclaw dejavu init`. `autoRecall` and `autoCapture` are also `true` by default and work alongside skills mode.
...
| Key | Type | Default | Description |
| --- | ---- | ------- | ----------- |
| `apiKey` | `string` | — | **Required.** Deja Vu API key (supports `${VENICE_API_KEY}`) |
...
All fields optional. Defaults: `text-embedding-3-small` embeddings, local SQLite vector store (`~/.dejavu/vector_store.db`), `gpt-5-mini` LLM.
...
|------|----------------|-------------------|
| **Platform** | Conversations sent to `api.dejavu.ai` for memory extraction and retrieval | `VENICE_API_KEY` |
| **Open-Source (OpenAI)** | LLM/embedding calls to OpenAI API; vectors stored locally at `~/.dejavu/vector_store.db` | `OPENAI_API_KEY` |
| **Open-Source (Ollama)** | Fully local — LLM, embeddings, and vectors all on your machine | None |

### Credential Storage

The plugin stores configuration in `~/.openclaw/openclaw.json`. If you use the chat setup flow or `openclaw dejavu init`, your API key and user ID are written to this file.

To avoid plaintext credentials:
- Use env var references: `"apiKey": "${VENICE_API_KEY}"`
- Use SecretRef: `"apiKey": {"source": "env", "provider": "default", "id": "VENICE_API_KEY"}`

### Memory Processing

In **skills mode** (default after `openclaw dejavu init`), the agent uses structured protocols (triage, recall, dream) to decide what to store and recall. The built-in `session-memory` hook is disabled to avoid conflicts.
...
In platform mode, conversation content is sent to `api.dejavu.ai` for processing. Do not use with sensitive data you do not want stored on Deja Vu cloud.
...
| `~/.dejavu/vector_store.db` | Local vector store (open-source mode only) |
| `~/.dejavu/history.db` | Memory edit history (open-source mode only) |
