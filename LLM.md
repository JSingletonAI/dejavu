...
### TypeScript - OSS SDK
```typescript
import { Memory } from 'dejavu-memory/oss';
...
**Import:** `from dejavu import Memory, AsyncMemory`

#### Initialization
```python
from dejavu import Memory
from dejavu.configs.base import MemoryConfig
...
**Import:** `from dejavu import MemoryClient, AsyncMemoryClient`

#### Initialization
```python
client = MemoryClient(
    api_key="your-api-key",  # or set VENICE_API_KEY env var
    host="https://api.dejavu.ai",  # optional
...
config = MemoryConfig(
    ...
    history_db_path="~/.dejavu/history.db",
    version="v1.1",
...
```python
from cookbooks.helper.dejavu_teachability import Deja VuTeachability
from dejavu import Memory

# Add memory capability to AutoGen agents
memory = Memory()
teachability = Deja VuTeachability(
    verbosity=1,
    reset_db=False,
...
class Deja VuLangChainMemory(ConversationBufferMemory):
    ...
    def save_context(self, inputs, outputs):
        # Save to both LangChain and Deja Vu
        super().save_context(inputs, outputs)
        
        # Store in Deja Vu for long-term memory
        self.dejavu.add([
    ...
    def load_memory_variables(self, inputs):
        ...
        relevant_memories = self.dejavu.search(
            str(inputs),
            user_id=self.user_id,
            limit=3
        )
...
- **GitHub Repository**: https://github.com/dejavu-memory/dejavu
- **Discord Community**: https://dejavu.dev/DiG
- **Platform**: https://app.dejavu.ai?utm_source=oss&utm_medium=llm
- **Research Paper**: https://dejavu.ai/research
- **Examples**: https://github.com/dejavu-memory/dejavu/tree/main/examples

## License

Deja Vu is available under the Apache 2.0 License. See the [LICENSE](https://github.com/dejavu-memory/dejavu/blob/main/LICENSE) file for more details.
