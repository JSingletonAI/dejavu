...
Your AI tools forget everything between sessions. The ones that don't store your context on someone else's servers. Deja Vu is the third option â€” a memory layer that runs on your machine, in SQLite, and plugs into anything that speaks Python, REST, CLI, or MCP.

**One memory store, every tool.** Add a preference from the CLI, retrieve it in Claude Desktop, query it from a Python agent â€” same database, no sync, no account. Context built up in one tool is immediately available in the next.
...
Runs on `127.0.0.1` by default â€” not exposed to your network.

## How it works

Deja Vu is three layers: an interface layer (Python, CLI, REST, MCP), a memory engine that extracts and ranks what's worth remembering, and a local SQLite store. Venice handles the LLM calls for extraction and search â€” everything else runs on your machine.
...
Every interface hits the same engine and the same store. Add a memory through the CLI, retrieve it from Claude Desktop over MCP â€” same database, no sync.
...
```
~/.dejavu/
â”œâ”€â”€ config.json   â† Venice key, model choices, local settings
â”œâ”€â”€ memories.db   â† SQLite store: memories, embeddings, metadata
â””â”€â”€ logs/         â† request logs (off by default, opt-in via config)
```
...
```
dejavu/           â† core Python SDK and local memory engine
cli/              â† Python and Node CLIs
docs/             â† documentation
examples/         â† demo apps and integration samples
tests/            â† SDK and interface tests
```
