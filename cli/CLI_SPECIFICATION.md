# dejavu CLI SDK Specification

Complete reference for the dejavu CLI. This document is the authoritative guide for any developer or AI agent working on this SDK.
...
dejavu CLI is the official command-line interface for [dejavu](https://dejavu.ai) -- the memory layer for AI agents. It lets developers and AI agents add, search, list, update, and delete memories via the dejavu Platform API from the terminal.

### Who is it for?

- Developers integrating dejavu into their workflows
...
The `cli/` directory provides the dejavu CLI in two languages with a shared specification for behavioral consistency.

| Language   | Directory  | Package Name  | Install Command            |
|------------|------------|---------------|----------------------------|
| TypeScript | `node/`    | `@dejavu/cli`   | `npm install -g @dejavu/cli` |
| Python     | `python/`  | `dejavu-cli`    | `pip install dejavu-cli`     |

Both implementations produce a binary named `dejavu` and provide **identical CLI behavior** -- same commands, same options, same output formats, same error messages.
...
| Entrypoint       | `dejavu = "dejavu_cli.app:main"`        | `"bin": { "dejavu": "./dist/index.js" }`  |
...
Interactive setup wizard for dejavu CLI.
...
- If `~/.dejavu/config.json` exists with an API key, the CLI warns and asks for confirmation before overwriting.
...
- Interactive mode: prints banner, prompts for API key (masked with `*`), prompts for default user ID (default: `dejavu-cli`), validates connection, saves config.
...
echo "text" | dejavu add -u alice
dejavu add --file msgs.json -u alice -o json
```
...
dejavu update abc-123 --metadata '{"key":"val"}'
...
1. **Single memory:** `dejavu delete <memory_id>` -- deletes one memory by ID. Cannot combine with `--all` or `--entity`.
2. **Bulk delete:** `dejavu delete --all [scope]` -- deletes all memories matching scope filters. Use `--project` with `--all` to wipe all memories project-wide (sends wildcard `*` entity IDs). Cannot combine with `<memory_id>` or `--entity`.
3. **Entity cascade:** `dejavu delete --entity [scope]` -- deletes the entity itself and all its memories. Cannot combine with `<memory_id>` or `--all`.
...
**Examples:**
```bash
dejavu delete abc-123-def-456              # single memory
dejavu delete --all -u alice --force        # all memories for user
dejavu delete --all --project --force       # project-wide wipe
dejavu delete --entity -u alice --force     # entity + all its memories
dejavu delete abc-123 --dry-run             # preview single delete
```
...
| Property         | Value |
|------------------|-------|
| Usage            | `dejavu entity list <entity_type>` |
| needsBackend     | Yes |
| needsConfig      | No |
| resolveIds       | No |
| resolveGraph     | No |
...
Delete an entity and ALL its memories (cascade). Also accessible via `dejavu delete --entity`.
...
Default: `https://api.dejavu.ai` (configurable via `--base-url`, `DEJAVU_BASE_URL`, or `platform.base_url` in config).
...
- Directory: `~/.dejavu/` (created with permissions `0700`)
- File: `~/.dejavu/config.json` (written with permissions `0600`)
...
| `DEJAVU_ENABLE_GRAPH`| `defaults.enable_graph`  | boolean | `false`               |

**Boolean parsing for `DEJAVU_ENABLE_GRAPH`:** Accepted truthy values are `"true"`, `"1"`, `"yes"` (case-insensitive). Everything else is `false`.
...
| `platform.api_key`       | string  | `""`                     | API key for dejavu Platform. |
...
- Config directory (`~/.dejavu/`): created with mode `0o700` (owner read+write+execute only).
- Config file (`~/.dejavu/config.json`): written with mode `0o600` (owner read+write only).
...
When both `--api-key` and `--user-id` are provided to `dejavu init`:
...
Every data-returning command supports `--output json` (or `--json` for `help`). This enables machine consumption by AI agents and scripts. JSON output goes to stdout while human-readable spinners/timing go to stderr, so piping `dejavu list -o json | jq .` works cleanly.
...
cat query.txt | dejavu search -u alice
echo "updated text" | dejavu update abc-123
```
...
```json
{
  ...
  "usage": "dejavu my-command <arg> [OPTIONS]",
  "needsBackend": true,
  "needsConfig": true,
  "resolveIds": true,
  "resolveGraph": false,
...
**Python** (`python/src/dejavu_cli/backend/base.py` and `platform.py`):
...
**Python** (`python/src/dejavu_cli/commands/`):
...
**Python** (`python/src/dejavu_cli/app.py`):
```python
...
def my_command(
    ...
    """Command description."""
    from dejavu_cli.commands.my_module import cmd_my_command
...
| Module        | Purpose |
|---------------|---------|
| **config**    | Load `~/.dejavu/config.json`, apply env var overrides, enforce precedence. Implement `load_config`, `save_config`, `ensure_config_dir`, `redact_key`, `get_nested_value`, `set_nested_value`. |
...
- [ ] Binary must be named `dejavu`

### 10.3 Package Metadata

Follow the naming conventions:
- Package description: "The official CLI for dejavu -- the memory layer for AI agents"
- Author: `dejavu.ai <founders@dejavu.ai>`
- License: Apache-2.0
- Keywords: `dejavu`, `memory`, `ai`, `agents`, `cli`
