...
as a vector store backend for Deja Vu's memory layer.
"""
...
class NeptuneAnalyticsConfig(BaseModel):
    ...
    for storing and retrieving memory embeddings in Deja Vu.
    
    Attributes:
        collection_name (str): Name of the collection to store vectors. Defaults to "dejavu".
        endpoint (str): Neptune Analytics graph endpoint URL or Graph ID for the runtime.
    """
    collection_name: str = Field("dejavu", description="Default name for the collection")
