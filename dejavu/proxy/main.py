...
from dejavu import Memory, MemoryClient
from dejavu.configs.prompts import MEMORY_ANSWER_PROMPT
from dejavu.memory.telemetry import capture_client_event, capture_event
...
class Dejavu:
    def __init__(
        ...
        if api_key:
            self.dejavu_client = MemoryClient(api_key, host)
        else:
            self.dejavu_client = Memory.from_config(config) if config else Memory()

        self.chat = Chat(self.dejavu_client)


class Chat:
    def __init__(self, dejavu_client):
        self.completions = Completions(dejavu_client)


class Completions:
    def __init__(self, dejavu_client):
        self.dejavu_client = dejavu_client
    ...
    def create(
        ...
        if isinstance(self.dejavu_client, Memory):
            capture_event("dejavu.chat.create", self.dejavu_client)
        else:
            capture_client_event("dejavu.chat.create", self.dejavu_client)
        return response
    ...
    def _fetch_relevant_memories(self, messages, user_id, agent_id, run_id, filters, top_k):
        ...
        return self.dejavu_client.search(
            query="\n".join(message_input),
            user_id=user_id,
            agent_id=agent_id,
            run_id=run_id,
    ...
    def _format_query_with_memories(self, messages, relevant_memories):
        # Check if self.dejavu_client is an instance of Memory or MemoryClient

        entities = []
        if isinstance(self.dejavu_client, dejavu.memory.main.Memory):
        ...
        elif isinstance(self.dejavu_client, dejavu.client.main.MemoryClient):
