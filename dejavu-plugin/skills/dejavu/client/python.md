# Deja Vu Python SDK Reference

Complete reference for the `dejavu-memory` Python package. Covers both the Platform client (managed API) and the Open Source self-hosted variant.
...
**Constructor:** `MemoryClient(api_key=None)`. If `api_key` is not provided, reads from `VENICE_API_KEY` environment variable. Raises `ValueError` if no key found.

- HTTP library: `httpx`
- Timeout: 300 seconds
- Base URL: `https://api.dejavu.ai`

### AsyncMemoryClient (Asynchronous)

```python
from dejavu import AsyncMemoryClient
...
**Import:** `from dejavu import Memory` (NOT `MemoryClient` -- that is the Platform client)
...
```python
from dejavu import AsyncMemory
...
|--------|--------------------------|----------------|
| **Import** | `from dejavu import MemoryClient` | `from dejavu import Memory` |
| **Auth** | API key required (`VENICE_API_KEY`) | No API key -- config-based |
| **Execution** | API calls to `api.dejavu.ai` | Local execution |
...
See the [v2 to v3 migration guide](https://docs.dejavu.ai/migration/oss-v2-to-v3) for full details.
