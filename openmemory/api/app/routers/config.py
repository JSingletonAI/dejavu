...
class VectorStoreProvider(BaseModel):
    ...
    # Below config can vary widely based on the vector store used. Refer https://docs.dejavu.ai/components/vectordbs/config
...
class ConfigSchema(BaseModel):
    ...
    dejavu: Optional[DejavuConfig] = None
...
def get_config_from_db(db: Session, key: str = "main"):
    ...
    if "dejavu" not in config_value:
        config_value["dejavu"] = default_config["dejavu"]
    else:
        # Ensure LLM config exists with defaults
        if "llm" not in config_value["dejavu"] or config_value["dejavu"]["llm"] is None:
            config_value["dejavu"]["llm"] = default_config["dejavu"]["llm"]
        
        # Ensure embedder config exists with defaults
        if "embedder" not in config_value["dejavu"] or config_value["dejavu"]["embedder"] is None:
            config_value["dejavu"]["embedder"] = default_config["dejavu"]["embedder"]
        
        # Ensure vector_store config exists with defaults
        if "vector_store" not in config_value["dejavu"]:
            config_value["dejavu"]["vector_store"] = default_config["dejavu"]["vector_store"]
...
async def update_configuration(config: ConfigSchema, db: Session = Depends(get_db)):
    ...
    # Update dejavu settings
    updated_config["dejavu"] = config.dejavu.dict(exclude_none=True)
...
@router.get("/dejavu/llm", response_model=LLMProvider)
async def get_llm_configuration(db: Session = Depends(get_db)):
    """Get only the LLM configuration."""
    config = get_config_from_db(db)
    llm_config = config.get("dejavu", {}).get("llm", {})
    return llm_config

@router.put("/dejavu/llm", response_model=LLMProvider)
async def update_llm_configuration(llm_config: LLMProvider, db: Session = Depends(get_db)):
    """Update only the LLM configuration."""
    current_config = get_config_from_db(db)
    
    # Ensure dejavu key exists
    if "dejavu" not in current_config:
        current_config["dejavu"] = {}
    
    # Update the LLM configuration
    current_config["dejavu"]["llm"] = llm_config.dict(exclude_none=True)
    
    # Save the configuration to database
    save_config_to_db(db, current_config)
    reset_memory_client()
    return current_config["dejavu"]["llm"]

@router.get("/dejavu/embedder", response_model=EmbedderProvider)
async def get_embedder_configuration(db: Session = Depends(get_db)):
    """Get only the Embedder configuration."""
    config = get_config_from_db(db)
    embedder_config = config.get("dejavu", {}).get("embedder", {})
    return embedder_config

@router.put("/dejavu/embedder", response_model=EmbedderProvider)
async def update_embedder_configuration(embedder_config: EmbedderProvider, db: Session = Depends(get_db)):
    ...
    current_config["dejavu"]["embedder"] = embedder_config.dict(exclude_none=True)
    
    # Save the configuration to database
    save_config_to_db(db, current_config)
    reset_memory_client()
    return current_config["dejavu"]["embedder"]

@router.get("/dejavu/vector_store", response_model=Optional[VectorStoreProvider])
async def get_vector_store_configuration(db: Session = Depends(get_db)):
    """Get only the Vector Store configuration."""
    config = get_config_from_db(db)
    vector_store_config = config.get("dejavu", {}).get("vector_store", None)
    return vector_store_config

@router.put("/dejavu/vector_store", response_model=VectorStoreProvider)
async def update_vector_store_configuration(vector_store_config: VectorStoreProvider, db: Session = Depends(get_db)):
    ...
    current_config["dejavu"]["vector_store"] = vector_store_config.dict(exclude_none=True)
    
    # Save the configuration to database
    save_config_to_db(db, current_config)
    reset_memory_client()
    return current_config["dejavu"]["vector_store"]
