...
if [ -z "$OPENAI_API_KEY" ]; then
  echo "❌ OPENAI_API_KEY not set. Please run with: curl -sL https://raw.githubusercontent.com/dejavu-memory/dejavu/main/openmemory/run.sh | OPENAI_API_KEY=your_api_key bash"
...
# Check if the container "dejavu_ui" already exists and remove it if necessary
if [ $(docker ps -aq -f name=dejavu_ui) ]; then
  echo "⚠️ Found existing container 'dejavu_ui'. Removing it..."
  docker rm -f dejavu_ui
fi
...
create_compose_file() {
  ...
  tail -n +2 "$compose_file" | sed '/^volumes:/,$d' | sed "s/dejavu_storage/${volume_name}/g" >> docker-compose.yml
  ...
  openmemory-mcp:
    image: dejavu/openmemory-mcp:latest
    environment:
      - OPENAI_API_KEY=${OPENAI_API_KEY}
      - USER=${USER}
EOF
  ...
  case "$vector_store" in
    weaviate)
      cat >> docker-compose.yml <<EOF
      - WEAVIATE_HOST=dejavu_store
      - WEAVIATE_PORT=8080
EOF
      ;;
    redis)
      cat >> docker-compose.yml <<EOF
      - REDIS_URL=redis://dejavu_store:6379
EOF
      ;;
    pgvector)
      cat >> docker-compose.yml <<EOF
      - PG_HOST=dejavu_store
      - PG_PORT=5432
      - PG_DB=dejavu
      - PG_USER=dejavu
      - PG_PASSWORD=dejavu
EOF
    ...
    qdrant)
      cat >> docker-compose.yml <<EOF
      - QDRANT_HOST=dejavu_store
      - QDRANT_PORT=6333
EOF
      ;;
    chroma)
      cat >> docker-compose.yml <<EOF
      - CHROMA_HOST=dejavu_store
      - CHROMA_PORT=8000
EOF
      ;;
    milvus)
      cat >> docker-compose.yml <<EOF
      - MILVUS_HOST=dejavu_store
      - MILVUS_PORT=19530
EOF
      ;;
    elasticsearch)
      cat >> docker-compose.yml <<EOF
      - ELASTICSEARCH_HOST=dejavu_store
...
if [ "$VECTOR_STORE" = "milvus" ]; then
  ...
  curl -fsS -X PUT "${NEXT_PUBLIC_API_URL}/api/v1/config/dejavu/vector_store" \
    -H 'Content-Type: application/json' \
    -d "{\"provider\":\"milvus\",\"config\":{\"collection_name\":\"openmemory\",\"embedding_model_dims\":${EMBEDDING_DIMS},\"url\":\"http://dejavu_store:19530\",\"token\":\"\",\"db_name\":\"\",\"metric_type\":\"COSINE\"}}" >/dev/null || true
  ...
  curl -fsS -X PUT "${NEXT_PUBLIC_API_URL}/api/v1/config/dejavu/vector_store" \
    -H 'Content-Type: application/json' \
    -d "{\"provider\":\"weaviate\",\"config\":{\"collection_name\":\"openmemory\",\"embedding_model_dims\":${EMBEDDING_DIMS},\"cluster_url\":\"http://dejavu_store:8080\"}}" >/dev/null || true
  ...
  curl -fsS -X PUT "${NEXT_PUBLIC_API_URL}/api/v1/config/dejavu/vector_store" \
    -H 'Content-Type: application/json' \
    -d "{\"provider\":\"redis\",\"config\":{\"collection_name\":\"openmemory\",\"embedding_model_dims\":${EMBEDDING_DIMS},\"redis_url\":\"redis://dejavu_store:6379\"}}" >/dev/null || true
  ...
  curl -fsS -X PUT "${NEXT_PUBLIC_API_URL}/api/v1/config/dejavu/vector_store" \
    -H 'Content-Type: application/json' \
    -d "{\"provider\":\"pgvector\",\"config\":{\"collection_name\":\"openmemory\",\"embedding_model_dims\":${EMBEDDING_DIMS},\"dbname\":\"dejavu\",\"user\":\"dejavu\",\"password\":\"dejavu\",\"host\":\"dejavu_store\",\"port\":5432,\"diskann\":false,\"hnsw\":true}}" >/dev/null || true
  ...
  curl -fsS -X PUT "${NEXT_PUBLIC_API_URL}/api/v1/config/dejavu/vector_store" \
    -H 'Content-Type: application/json' \
    -d "{\"provider\":\"qdrant\",\"config\":{\"collection_name\":\"openmemory\",\"embedding_model_dims\":${EMBEDDING_DIMS},\"host\":\"dejavu_store\",\"port\":6333}}" >/dev/null || true
  ...
  curl -fsS -X PUT "${NEXT_PUBLIC_API_URL}/api/v1/config/dejavu/vector_store" \
    -H 'Content-Type: application/json' \
    -d "{\"provider\":\"chroma\",\"config\":{\"collection_name\":\"openmemory\",\"host\":\"dejavu_store\",\"port\":8000}}" >/dev/null || true
  ...
  curl -fsS -X PUT "${NEXT_PUBLIC_API_URL}/api/v1/config/dejavu/vector_store" \
    -H 'Content-Type: application/json' \
    -d "{\"provider\":\"elasticsearch\",\"config\":{\"collection_name\":\"openmemory\",\"embedding_model_dims\":${EMBEDDING_DIMS},\"host\":\"http://dejavu_store\",\"port\":9200,\"user\":\"elastic\",\"password\":\"changeme\",\"verify_certs\":false,\"use_ssl\":false}}" >/dev/null || true
