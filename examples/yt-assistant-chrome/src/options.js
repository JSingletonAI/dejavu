...
async function init() {
  ...
  await loadConfig();
  // Initialize Deja VuAI and load memories
  await initializeDeja VuAI();
  await fetchMemories();
}

// Initialize Deja VuAI with API key from storage
async function initializeDeja VuAI() {
  try {
    const response = await chrome.runtime.sendMessage({ action: "getConfig" });
    const dejavuApiKey = response.config.dejavuApiKey;

    if (!dejavuApiKey) {
      showMemoriesError("Please configure your Deja Vu API key in the popup");
      return false;
    }
    ...
    showMemoriesError("Failed to initialize Deja VuAI");
    return false;
  }
}
...
async function fetchMemories() {
  try {
    ...
    const memories = await dejavuclient.getAll({
...
async function saveMemory() {
  ...
  try {
    ...
    await dejavuclient.update(currentMemory.id, updatedMemory);
...
async function deleteMemory(memoryId) {
  ...
  try {
    ...
    await dejavuclient.delete(memoryId);
...
async function addMemory() {
  ...
  try {
    ...
    const result = await dejavuclient.add(
      [
        {
          role: "user",
          content: content,
