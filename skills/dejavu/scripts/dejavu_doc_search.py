#!/usr/bin/env python3
"""
Deja Vu Documentation Search Agent (Mintlify-based)
On-demand search tool for querying Deja Vu documentation without storing content locally.

This tool leverages Mintlify's documentation structure to perform just-in-time
retrieval of technical information from docs.dejavu.ai.

Usage:
    python dejavu_doc_search.py --query "how to add graph memory"
    python dejavu_doc_search.py --query "filter syntax for categories"
    python dejavu_doc_search.py --page "/platform/features/graph-memory"
    python dejavu_doc_search.py --index
    python dejavu_doc_search.py --query "webhook events" --section platform
...
Purpose:
    ...
    - Search across the full Deja Vu documentation site
"""
...
DOCS_BASE = "https://docs.dejavu.ai"
...
def fetch_url(url: str) -> str:
    ...
    req = urllib.request.Request(url, headers={"User-Agent": "Deja VuDocSearchAgent/1.0"})
    try:
...
def search_docs(query: str, section: str | None = None) -> dict:
    """
    Search Deja Vu documentation using Mintlify's search API.
...
def main():
    parser = argparse.ArgumentParser(description="Search Deja Vu documentation on demand")
