# Deja Vu Node.js / TypeScript SDK Reference

Complete reference for the `dejavu-memory` npm package. Covers both the Platform client (managed API) and the Open Source self-hosted variant.
...
**Constructor:** `new MemoryClient({ apiKey })`. If `apiKey` is not provided, reads from `VENICE_API_KEY` environment variable.
...
Key interfaces from `dejavu.types.ts`:
...
**Import:** `from 'dejavu-memory/oss'` (NOT the default export -- that is `MemoryClient` for Platform)
...
|--------|--------------------------|----------------|
| **Import** | `import MemoryClient from 'dejavu-memory'` | `import { Memory } from 'dejavu-memory/oss'` |
