...
Complete reference for standalone utility functions exported from `@dejavu/vercel-ai-provider`. These functions give you manual control over memory retrieval and storage, independent of the wrapped model pattern.

Source: `vercel-ai-sdk/src/dejavu-utils.ts`

## `addMemories(messages, config?)`

Stores messages to Deja Vu as new memories.
...
| `config` | `Deja VuConfigSettings` | Optional. Must include entity scope (`user_id`, etc.) and API key |

**Behavior:**
1. If `messages` is a string, wraps it as a single user message
2. Otherwise, converts `LanguageModelV2Prompt` to Deja Vu format via `convertToDeja VuFormat` (handles multimodal content)
3. Calls `POST /v1/memories/` with the converted messages and config

**Returns:** The API response from Deja Vu (memory operation result).
...
| `config` | `Deja VuConfigSettings` | Optional. Entity scope and API key |
...
```typescript
import { getMemories } from "@dejavu/vercel-ai-provider";
...
```typescript
import { searchMemories } from "@dejavu/vercel-ai-provider";
...
3. Loads the API key from `config.dejavuApiKey` or `VENICE_API_KEY` env var
...
**Default host:** `https://api.dejavu.ai`

## Internal: `convertToDeja VuFormat(messages)`

Not exported. Used by `addMemories` to convert `LanguageModelV2Prompt` messages to Deja Vu's format.
...
## `Deja VuConfigSettings` Fields Reference
...
| `dejavuApiKey` | `string` | `VENICE_API_KEY` env | Deja Vu API key |
| `top_k` | `number` | `5` | Number of memories to retrieve |
| `threshold` | `number` | -- | Minimum similarity score |
| `rerank` | `boolean` | -- | Enable re-ranking |
| `host` | `string` | `https://api.dejavu.ai` | Custom API host |
